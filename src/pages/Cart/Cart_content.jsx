...
const Cart_content = ({ num, list, totalPrice, increase_quantity, decrease_quantity, delete_book_order }) => {
    ...
    const sumPrice = totalPrice || list.reduce((total, book) => total + book.price * book.quantity, 0);
        ...
        <div className="pt-20 p-4 flex h-screen mx-8"> {/* Set height to screen height */}
            ...
            <div className="flex-1 overflow-y-auto h-full"> {/* Flex 1 for responsive layout, h-full to take full height */}
                ...
                <div className="flex flex-col space-y-4">
                    {list.length > 0 ? (
                        list.map((book) => (
                            <div key={book.id} className="flex bg-gray-100 p-4 rounded-md shadow-md">
                                <img
                                    src={book.image}
                                    alt={book.title}
                                ...
                                <div className="flex flex-col space-y-2 ml-4 w-full">
                                    <h3 className="text-3xl font-bold text-left text-gray-900">{book.title}</h3>
                                ...
                                <div className="flex flex-col space-y-2 ml-4 w-full justify-start items-end gap-32">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <div className="inline-flex shadow-sm bg-gray-200 items-center rounded-3xl border-4 border-orange-400">
                                            <button onClick={()=>decrease_quantity(book.id)} className="px-4 py-2 text-gray-600 hover:text-gray-900 font-semibold rounded-s-3xl hover:bg-gray-300">-</button>
                                            <span className="px-4 py-2   text-base text-gray-900 font-semibold ">{book.quantity}</span>
                                            <button onClick={()=>increase_quantity(book.id)} className="px-4 py-2 text-gray-600 hover:text-gray-900 font-semibold rounded-e-3xl hover:bg-gray-300">+</button>
                                        </div>

                                        <button onClick={()=>delete_book_order(book.id)}>
